...
//        case .primary: return .percentage(isPortrait(context) ? 0.25 : 0.195)
        case .primary: return .percentage(isPortrait(context) ? 0.21 : 0.195)
    ...
    open func lowerLeadingActions(
        ...
        if context.keyboardLocale?.shiftButton ?? true {
            if let switcher = keyboardSwitchActionForBottomInputRow(for: context) {
                result.append(switcher)
            }
        }
        
        if context.keyboardLocale?.marginBottom ?? true {
