...
//            .sink(receiveValue: { [weak self] _ in self?.refresh() })
            .sink(receiveValue: { [weak self] _ in self?.refreshActiveApp() })
            .store(in: &cancellables)
        textPublisher
            .delay(for: 0.5, scheduler: RunLoop.main)
//            .sink(receiveValue: { [weak self] _ in self?.refresh() })
            .sink(receiveValue: { [weak self] _ in self?.refreshActiveKeyboard() })
            .store(in: &cancellables)
        refresh()
    }
...
//        isKeyboardEnabled = isKeyboardEnabled(withBundleId: bundleId)
    ...
    public func refreshActiveKeyboard() {
        self.isKeyboardActive = isKeyboardActive(withBundleId: bundleId)
    }
}
...
private extension KeyboardStateContext {
    ...
    var keyboardPublisher: NotificationCenter.Publisher {
